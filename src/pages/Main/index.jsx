...
const Main = () => {
    //hoverlanan ülkenin bilgisini state aktarmak
  ...
  return (
    <div className= 'h-[calc(100vh-74px)] bg-zinc-800 text-white overflow-hidden flex flex-col justify-center items-center md:pt-20 wrapper'>
        <h1 className='px-6 mt-20'>Detay Görüntüle{" "}
            {/**EĞERKİ İÇİNDE PROPERTİES DEĞERİ VARSA VE ONUN İÇİNDE NAME DEĞERİ VARSA EKRANA BAS YOKSA ÜLKE SEÇİNİ BAS  */}
      ...
      <Geographies geography={geoUrl}>
        {({ geographies }) =>
          geographies.map((geo) => (
            <Link to={`/detail?code=${geo.id}`}> {/**DETAY SAYFASINA YÖNLENDİRME */}
            <Geography 
